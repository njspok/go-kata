...
// Add provides a mock function with given fields: name, _a1, direction
func (_m *MockAuditRep) Add(name string, _a1 time.Time, direction Direction) error {
	ret := _m.Called(name, _a1, direction)

	var r0 error
	if rf, ok := ret.Get(0).(func(string, time.Time, Direction) error); ok {
		r0 = rf(name, _a1, direction)
	} else {
		r0 = ret.Error(0)
	}
...
//   - _a1 time.Time
//   - direction Direction
func (_e *MockAuditRep_Expecter) Add(name interface{}, _a1 interface{}, direction interface{}) *MockAuditRep_Add_Call {
	return &MockAuditRep_Add_Call{Call: _e.mock.On("Add", name, _a1, direction)}
}

func (_c *MockAuditRep_Add_Call) Run(run func(name string, _a1 time.Time, direction Direction)) *MockAuditRep_Add_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(string), args[1].(time.Time), args[2].(Direction))
	})
	return _c
}
